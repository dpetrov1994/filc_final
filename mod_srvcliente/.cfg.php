...
$nomeTabela="srv_clientes";
$nomeColuna="cliente";
$innerjoin=" left join srv_clientes_CustomerLedgerAccount using(PartyID)";
$add_sql.="  ";
...
$colunasParaMostrarNaTabela=[
    "checkbox" => "checkbox",
    "OrganizationName" => "link|text::<i class='fa fa-user'>|class::text-left|style::padding-left:15px",
    "classificacao" => "text::Classificação",
    "pais" => "text::País",

    ///////////////////////////////////////////////
    "pendente" => "text::Valor Pendente|class::text-right",
    //"funcionalidades"=>"funcionalidades", // apagar esta linha se não quiser mostrar
];

if(isset($_GET['excel'])){
    $colunasParaMostrarNaTabela=[];
    $sql2="SHOW COLUMNS FROM $nomeTabela";
    $result2=runQ($sql2,$db,"colunas");
    while ($row2 = $result2->fetch_assoc()) {
        $colunasParaMostrarNaTabela[$row2['Field']]="text::".$row2['Field'];
    }
}
