...
;( function( window ) {
	...
	FForm.prototype._addControls = function() {
		...
		this.ctrlContinue = createElement( 'button', { cName : 'fs-continue', inner : '<span class="hidden-xs">Seguinte</span>', appendTo : this.ctrls } );
	...
	FForm.prototype._validade = function() {
		...
		if(input.name=="codigo_verificacao"){
			if(input.value!=parseInt(document.getElementById("codigo").value)/2){
                error = 'NOVAL';
			}
		}
    ...
    function get_horas(form){
        var data=$('input[name=data_inicio]:checked').val();
        var xmlhttp = new XMLHttpRequest();
        xmlhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
            	//alert(this.responseText);
                document.getElementById("horas").innerHTML = this.responseText;
                form._initEvents();
            }
        };
        xmlhttp.open("GET", "_get_horas.php?data=" + data, true);
        xmlhttp.send();
    }
    ...
    function enviar_codigo(){
        ...
        xmlhttp.open("GET", "_enviar_codigo.php?email=" + email, true);
        xmlhttp.send();
    }
...
$( "input:checked" ).each(function () {
	$(this).find("label").find("span").css("opacoty",1);
});
