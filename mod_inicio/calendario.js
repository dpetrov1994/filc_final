...
function mostraModalDetalhesEvento(id_evento) {
    ...
    xmlhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            openModal('#modal-detalhes-assistencia');

            $('#modal-detalhes-assistencia .modal-body-content').html(this.responseText);
            initforms();
        }
    };

    xmlhttp.open("GET", "dashboard.php?ver_detalhes_assistencia=" + id_evento, true);
...
$('#calendar').fullCalendar({
    ...
    dayClick: function(date, jsEvent, view) {
        ...
        if($(this).hasClass("bg-info")){
            ...
            var data_inicio=date.format('YYYY-MM-DDThh:mm:ss');
            data_inicio = data_inicio.replace('A','T');



            getHtmlCriarAjax('#modal-criar-assistencia','mod_assistencias', data_inicio);
