	...
	*/
	class TCPDF {
		...
		public function Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false) {
			...
			//$file=str_replace('orcamentos', 'produtos',$file);


		/*	if (!file_exists($file)) {

				foreach($_GET['linhas'] as $linha){
					if($linha['fotoproduto'] == ""){
						continue;
					}
					...
					/*$foto_produto_atual = explode('"', $linha['fotoproduto']);
					$foto_produto_atual = explode('/',$foto_produto_atual[1]);
					$id_produto = $foto_produto_atual[3];
					$nome_foto_produto =  $foto_produto_atual[4];
				}

				$file=explode('/',$file);
				$file="../_contents/produtos/".$id_produto."/".$file[4];
		...
		protected function _parsejpeg($file) {
			...
			$file="../_contents/orcamentos/".$_GET['id_orcamento']."/".$file[4];
			...
			if (!empty($this->form_obj_id) OR ($this->sign AND isset($this->signature_data['cert_type']))) {
				...
				if (isset($this->annotation_fonts) AND !empty($this->annotation_fonts)) {
					...
					foreach ($this->annotation_fonts as $font => $fontkey) {
 						...
 						$out .= ' /F'.($fontkey).' '.$this->font_obj_ids[$font].' 0 R';
					}
					$out .= ' >> >>';
				}
