...
class Imap {
    ...
    public function getMessages($withbody = true) {
        ...
        for ($i=1;$i<=$count;$i++) {
            ...
            if($email!=0){ //mensagem não aberta
                $emails[]=$email;
            }
        }
    ...
    protected function formatMessage($id, $withbody=true) {
        ...
        if($header->Unseen!='U'){ //para returnar só as não lidas
            return 0;
        }
        ...
        $sender_address=$header->sender[0]->mailbox."@".$header->sender[0]->host;
        $sender_name="";
        if(isset($header->sender[0]->personal)){
        ...
        $email = array(
            ...
            'from_array'      => $header->from,
            'to_email_address'      => $to_address,
            'to_name'      => $to_name,
            'from_email_address'      => $from_address,
            'from_name'      => $from_name,
            'sender_array'      => $header->sender,
            ...
            foreach ($attachments as $val) {
                ...
                foreach ($val as $k => $t) {
                    if ($k == 'name') {
                        ...
                        if(!isset($decodedName[0]->text)){
                            $convertToUtf8="noname";
                        }else{
                            $convertToUtf8=$decodedName[0]->text;
                        }
                        $t = $this->convertToUtf8($convertToUtf8);
                    }
                    $arr[$k] = $t;
                }
        ...
        if (isset($part->parts)) {
            ...
            if (in_array(strtolower($part->disposition), array('attachment', 'inline'))) {
                ...
                if(!isset($part->dparameters[0]->value)){
                    ...
                    $tmp_value=$part->dparameters[0]->value;
                }
