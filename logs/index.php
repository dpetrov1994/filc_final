...
if(isset($_GET['id_utilizador'])){
    ...
    $add_sql.=" and $nomeTabela.id_utilizador=$id_utilizador";
}

if(isset($_GET['nome_tabela'])){
    $nome_tabela=$db->escape_string($_GET['nome_tabela']);
    $add_sql.=" and $nomeTabela.nome_tabela='$nome_tabela'";

    if(isset($_GET['id_item'])){
        $id_item=$db->escape_string($_GET['id_item']);
        $add_sql.=" and $nomeTabela.id_item='$id_item'";
    }
...
if($tr>0){
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        while ($row2 = $result2->fetch_assoc()) {
            $tbody=str_replace("_nome_utilizador_","<a href='index.php_addUrl_&id_utilizador=".$row['id_utilizador']."'>".$row2['nome_utilizador']."</a>",$tbody);
        }
        $tbody=str_replace("_nome_utilizador_","<a href='index.php_addUrl_&id_utilizador=".$row['id_utilizador']."'>[AUTO]</a>",$tbody);
